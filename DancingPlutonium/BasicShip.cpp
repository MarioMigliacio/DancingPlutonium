...
{
	float randomX = _rt.getSize().x - sprite.getGlobalBounds().width / 2.0f;
	randomX = RandomIntRange(sprite.getGlobalBounds().width / 2.0f, randomX) % (int)randomX;

	SetPosition(sf::Vector2f(randomX, 16.0f));
}
